...
const HelpCenter = () => {
  ...
  const handleTicketSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setFormData({ subject: '', category: 'Payments', message: '' });
        fetchTickets();
      }
    } catch (error) {
  ...
  const faqs = [
    {
      category: 'Course Access & Progress',
      items: [
        { q: 'How do I access my enrolled courses?', a: 'Go to the "My Learning" tab from the sidebar. You will see all your purchased and enrolled courses there.' },
        { q: 'Why is my course progress not updating?', a: 'Make sure to mark each lesson as completed. If it still doesn\'t update, try refreshing your browser or clearing your cache.' },
        { q: 'Where can I download my certificate?', a: 'Once a course reaches 100% progress, a certificate will be issued. You can view and download it under the "Certificates" tab.' }
      ]
    },
    {
      category: 'Payments & Refunds',
      items: [
        { q: 'Which payment options are supported?', a: 'We support all major credit/debit cards, UPI, net banking, and wallets via Razorpay secure checkout.' },
        { q: 'What is the refund policy?', a: 'Please contact support within 7 days of course purchase for any refund requests, subject to our terms of service.' }
      ]
    }
  ];

  const categories = [
    { name: 'Payments', icon: HelpCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { name: 'Technical Issues', icon: HelpCircle, color: 'text-purple-600', bg: 'bg-purple-50' },
    { name: 'Student Management', icon: HelpCircle, color: 'text-blue-600', bg: 'bg-blue-50' },
    { name: 'Other', icon: HelpCircle, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
  ...
  return (
    <div className="animate-fade-in space-y-12">
      {/* Hero Section */}
      <div className="relative overflow-hidden bg-purple-600 rounded-[2.5rem] p-10 lg:p-14 text-center text-white shadow-xl shadow-purple-100">
        <div className="relative z-10 max-w-2xl mx-auto space-y-6">
          <h2 className="text-4xl lg:text-5xl font-black">Student Help Center</h2>
          <p className="text-purple-100">Browse frequently asked questions or raise a support ticket directly with our administration.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {categories.map((cat, i) => (
              <div key={i} className="p-6 bg-white rounded-2xl border border-slate-100 shadow-sm text-center flex flex-col items-center gap-3">
              ...
              {faqs.map((group, gIdx) => (
                <div key={gIdx} className="space-y-3">
                  ...
                  {group.items.map((faq, fIdx) => {
                    ...
                    return (
                      <div key={id} className={`bg-white rounded-xl border transition-all ${isOpen ? 'border-purple-200 shadow-sm' : 'border-slate-100'}`}>
                        <button 
          ...
          <AnimatePresence mode="wait">
            {!selectedTicket ? (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-purple-50 text-purple-600 font-bold text-xs rounded-xl hover:bg-purple-100 transition-colors"
                  >
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden min-h-[350px] flex flex-col">
                  <div className="p-3 border-b border-slate-50">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-slate-50 border-none rounded-xl text-xs focus:ring-1 focus:ring-purple-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="divide-y divide-slate-50 overflow-y-auto max-h-[400px] flex-1">
                    {fetchingTickets ? (
                      ...
                      >
                        ...
                        <h4 className="font-bold text-slate-800 text-sm group-hover:text-purple-600 transition-colors line-clamp-1">{ticket.subject}</h4>
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 20 }}
                className="bg-white rounded-2xl border border-slate-100 shadow-lg overflow-hidden flex flex-col h-[550px]"
              >
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  <div className="bg-purple-50/50 p-4 rounded-xl border border-purple-100/50 space-y-2">
                    <span className="text-[9px] font-bold text-purple-600 uppercase tracking-wider">{selectedTicket.category}</span>
                ...
                <form onSubmit={handleSendReply} className="p-3 border-t border-slate-100 bg-slate-50 flex items-center gap-2">
                  <input 
                    ...
                    className="flex-1 px-4 py-2 text-xs bg-white border border-slate-200 rounded-xl focus:ring-1 focus:ring-purple-500 focus:outline-none disabled:bg-slate-100"
                  />
                  <button 
                    type="submit" 
                ...
                <form onSubmit={handleTicketSubmit} className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold focus:ring-1 focus:ring-purple-500 focus:outline-none"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold focus:ring-1 focus:ring-purple-500 focus:outline-none appearance-none"
                    >
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold focus:ring-1 focus:ring-purple-500 focus:outline-none h-32 resize-none"
                      required
                    />
                  </div>
                  ...
                  <button 
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-purple-600 text-white font-bold rounded-xl shadow-md shadow-purple-100 hover:bg-purple-700 transition-all flex items-center justify-center gap-1.5 active:scale-95 disabled:opacity-50"
                  >
