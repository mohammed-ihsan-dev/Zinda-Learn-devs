...
const EditCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(isAdmin ? `/admin/courses/${id}` : `/instructor/courses/${id}`);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <Loader2 className={`w-10 h-10 animate-spin mb-4 ${isAdmin ? 'text-indigo-400' : 'text-purple-600'}`} />
        <p className={`font-medium ${isAdmin ? 'text-slate-400' : 'text-slate-500'}`}>Loading course data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in pb-20">
      <div className="mb-10">
        <Link 
          to={isAdmin ? `/admin/courses/${id}` : `/instructor/courses/${id}`} 
          className={`flex items-center gap-2 text-slate-400 ${isAdmin ? 'hover:text-indigo-405 hover:text-indigo-400' : 'hover:text-purple-600'} transition-colors text-xs font-bold uppercase tracking-widest mb-6 group`}
        >
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
          Back to Course Detail
        </Link>
        <h1 className={`text-4xl font-bold mb-2 tracking-tight ${isAdmin ? 'text-slate-100' : 'text-slate-900'}`}>Edit Course</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-8">
        ...
        <div className={`flex flex-col sm:flex-row justify-between items-center pt-10 gap-6 border-t ${
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <button 
              type="button"
              onClick={() => navigate(isAdmin ? `/admin/courses/${id}` : `/instructor/courses/${id}`)}
              className={isAdmin 
            ...
            <button 
              type="submit"
              disabled={saving}
              className={`flex-1 sm:flex-none px-12 py-4 text-white text-xs font-bold rounded-2xl transition-all active:scale-95 disabled:opacity-50 ${
                isAdmin 
